...
export default function ContentFour() {
  ...
  const [animationLeft] = useScrollFadeIn("left");
  const [animationRight] = useScrollFadeIn("right");

  return (
    <Motion.div
      {...animation}
      className="w-full min-h-[80vh] py-16 md:py-24 bg-amber-50 flex items-center overflow-x-hidden"
    >
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-8 items-center">
        {/* Wedding Party Section */}
        <Motion.div variants={animationLeft.variants}>
          <div className="text-center flex flex-col items-center p-6 bg-white rounded-2xl shadow-xl transition-transform duration-300 hover:-translate-y-2">
            ...
            <p className="mt-2 text-gray-600 font-cormorant max-w-md">
              We're so grateful to have these amazing people standing by our
              side. Get to know our bridesmaids and groomsmen!
            </p>
        ...
        {/* RSVP Section */}
        <Motion.div variants={animationRight.variants}>
          <div className="text-center flex flex-col items-center p-8 bg-emerald-800 text-white rounded-2xl shadow-xl transition-transform duration-300 hover:-translate-y-2">
            ...
            <p className="mt-2 font-cormorant max-w-md">
              Your presence at our wedding is the greatest gift of all. Please
              let us know if you can celebrate with us by October 31st, 2025.
            </p>
            <Link
              to="/schedule#rsvp"
